...
|---|---|
| `DATABASE_URL` | Supabase project → Project Settings → Database → connection string (hostname like `aws-0-<region>.pooler.supabase.com`, username `postgres.<project-ref>`). Prefer the **Session pooler (port 5432)** if it authenticates — it supports both normal runtime queries and `prisma db push`/migrate. **As of 2026-07-24 this project's session pooler (port 5432) started returning `P1000: Authentication failed` even with a freshly-reset password**, while the same credentials authenticate fine on the **Transaction pooler (port 6543)** — likely a Supabase-side change (IPv4 add-on now required for session pooler on some tiers, or similar). If session pooler auth fails, use port 6543 for day-to-day app runtime (normal Prisma queries work fine over it), and only switch to port 5432 (or the direct connection) transiently when you actually need to run `prisma db push`/`migrate` — those hang indefinitely on port 6543 (PgBouncer transaction-mode incompatibility with Prisma's schema-engine advisory locks). Do not use the direct-connection hostname (`db.xxxx.supabase.co`) from this sandbox — it's IPv6-only and unreachable. |
